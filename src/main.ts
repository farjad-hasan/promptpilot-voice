...
const urduHotkeyText = document.getElementById("urdu-hotkey-text")!;
const vocabPrompt = document.getElementById("vocabulary-prompt") as HTMLTextAreaElement;
const vocabCharCount = document.getElementById("vocab-char-count")!;
const vocabWarning = document.getElementById("vocab-warning")!;
const pasteDirectBtn = document.getElementById("paste-direct")!;
...
async function loadSettings() {
  ...
  // Urdu hotkey
  urduHotkeyText.textContent = currentSettings.urduHotkey;

  // Vocabulary prompt
  vocabPrompt.value = currentSettings.vocabularyPrompt;
  updateCharCounter();

  // Paste method
  setPasteMethod(currentSettings.pasteMethod);
}
...
function updateCharCounter() {
  const len = vocabPrompt.value.length;
  vocabCharCount.textContent = String(len);
  vocabWarning.classList.toggle("hidden", len <= 500);
}

function setPasteMethod(method: string) {
  currentSettings.pasteMethod = method;
  pasteDirectBtn.classList.toggle("active", method === "direct");
  pasteClipboardBtn.classList.toggle("active", method === "clipboard");
}
...
async function saveSettings() {
  ...
  currentSettings.vocabularyPrompt = vocabPrompt.value;
...
vocabPrompt.addEventListener("input", updateCharCounter);
vocabPrompt.addEventListener("change", saveSettings);

pasteDirectBtn.addEventListener("click", () => {
  setPasteMethod("direct");
  saveSettings();
});

pasteClipboardBtn.addEventListener("click", () => {
  setPasteMethod("clipboard");
  saveSettings();
});

// Listen for language mode to update status label
let currentLanguage = "en";
listen<string>("language-mode", (event) => {
  currentLanguage = event.payload;
  if (currentLanguage === "ur" && statusText.textContent === "Recording...") {
    statusText.textContent = "Recording (Urdu)...";
  }
});
