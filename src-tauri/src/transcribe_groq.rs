...
mod tests {
    ...
    #[tokio::test]
    async fn test_urdu_language_param_accepted() {
        let path = PathBuf::from("/tmp/test.wav");
        let result = transcribe_groq("", &path, "ur", "Claude Code").await;
        assert!(result.is_err());
        // Errors on empty key before HTTP call — language param accepted structurally
    }
}
